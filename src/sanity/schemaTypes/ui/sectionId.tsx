...
export default [
  defineField({
    ...
    description:
      'The Section ID is a unique identifier used to link to specific sections of the page.',
    validation: (Rule) => [
      Rule.custom((value, context) => {
        if (!value) return true;
        if (value?.startsWith('#'))
          return 'Section ID cannot start with a "#" symbol. It has to be just a string.';
        ...
        if (isDuplicate)
          return 'This section ID is already used in another component. Section IDs must be unique.';
        return true;
      }),
      Rule.custom((value) => {
        if (!value) return true;
